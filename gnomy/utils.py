...
def get_grib_hour_data(grib_dt, search_string_0h, search_string_1h, latitude, longitude):
...
def get_grib_data(grib_datetimes, latitude, longitude, search_string_0h, search_string_1h, n_jobs, cache_dir):
    ...
    def grib_download_wrapper(grib_datetime):
        ...
        df = get_grib_hour_data(grib_datetime, search_string_0h, search_string_1h, latitude, longitude)
        if df is not None:
            df.to_csv(save_path)
            return None
        else:
...
def combine_cache_files(
    ...
    all_cache_file_paths = [os.path.join(cache_dir, i.strftime("%Y%m%d%H%M.csv")) for i in all_dates]
...
def calculate_solar_zenith_angle(
    ...
    dt_index_resampled = pd.date_range(
        original_datetime_index[0] - offset_timedelta, original_datetime_index[-1], freq='5Min'
        )
    sp = solarposition.get_solarposition(dt_index_resampled, latitude, longitude)
    zenith_df = sp["zenith"].resample(offset_timedelta, label="right").mean()[original_datetime_index[0]:original_datetime_index[-1]]
...
def cloud_cover_to_opaque_sky_cover(lcc, mcc, hcc, tcc, tcc_translucent_ratio=0.5):
    ...
    """
    translucent_cloud_cover = np.clip(tcc - lcc - mcc, 0, 100) * tcc_translucent_ratio  # estimate of tcc that is only hcc.
...
def get_coordinate_projections(
    date,
    latitude, 
    longitude
    ):
...
def get_albedo(latitude, longitude):
    ...
    ds = H.xarray(":VGTYP:")
    distances = (ds.longitude.values - longitude)**2 + (ds.latitude.values - latitude)**2
...
def parse_weather_code(
    accumulated_precipitation, freezing_rain, ice_pellets, lightning, rain, snow,
    pct_frozen_precipitation, visibility, wind_gust_speed, smoke):
    """
    ...
    def code3_combined_rain(accumulated_precipitation, pct_freezing_precipitation, visibility, wind_gust_speed):
        """
    ...
    if accumulated_precipitation > 0:
        ...
        if rain:
            ...
            weather_code[2] = code3_combined_rain(accumulated_precipitation, pct_frozen_precipitation, visibility, wind_gust_speed)
        
        # code 4
        if snow:
            weather_code[3] = code4_snow_ice(accumulated_precipitation, ice_pellets)
            weather_code[4] = code5_snow_showers(accumulated_precipitation, wind_gust_speed, ice_pellets)
